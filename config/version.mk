...
# Evilness Preloaded
ifeq ($(BUILD_WITH_EVILNESS),true)
IS_PHONE := true
include vendor/gapps/config.mk

DEVILIAN_EDITION := EVILNESS
...
DEVILIAN_DISPLAY_VERSION := DevilianOS-$(DEVILIAN_MOD_VERSION)-$(DEVILIAN_BUILD_TYPE)
else
DEVILIAN_EDITION := GOODNESS
DEVILIAN_BUILD_DATETIME := $(shell date +%s)
DEVILIAN_BUILD_DATE := $(shell date -d @$(DEVILIAN_BUILD_DATETIME) +"%Y%m%d-%H%M%S")
