...
function setup_vendor() {
    ...
    export DEVILIAN_ROOT="$3"
    if [ ! -d "$DEVILIAN_ROOT" ]; then
        echo "\$DEVILIAN_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$DEVILIAN_ROOT/$OUTDIR" ]; then
        mkdir -p "$DEVILIAN_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$DEVILIAN_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$DEVILIAN_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$DEVILIAN_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The DevilianOS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The DevilianOS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The DevilianOS Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The DevilianOS Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The DevilianOS Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$DEVILIAN_ROOT"/vendor/devilian/build/tools/smali/baksmali.jar
        export SMALIJAR="$DEVILIAN_ROOT"/vendor/devilian/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$DEVILIAN_ROOT"/vendor/devilian/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$DEVILIAN_ROOT"/vendor/devilian/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$DEVILIAN_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$DEVILIAN_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$DEVILIAN_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$DEVILIAN_ROOT"/vendor/devilian/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$DEVILIAN_ROOT"/"$OUTDIR"/radio
