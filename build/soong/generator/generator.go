...
func init() {
	android.RegisterModuleType("devilian_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = devilianExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = devilianExpandVariables(ctx, dep_file)
	...
	cmd := devilianExpandVariables(ctx, String(g.properties.Cmd))
